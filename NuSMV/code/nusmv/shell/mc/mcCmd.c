...
int CommandCheckCtlgSpec(NuSMVEnv_ptr env, int argc, char** argv);
...
static int UsageCheckCtlgSpec(const NuSMVEnv_ptr env);
...
void Mc_Init(NuSMVEnv_ptr env)
{
  ...
  Cmd_CommandAdd(env, "check_gctlspec", CommandCheckCtlgSpec, 0, true);
...
int CommandCheckCtlgSpec(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  while ((c = util_getopt(argc,argv,"hmo:n:p:")) != EOF) {
    switch (c) {
      case 'h': return UsageCheckCtlgSpec(env);
      case 'n':
      {
        if (formula != NIL(char)) return UsageCheckCtlgSpec(env);
        if (prop_no != -1) return UsageCheckCtlgSpec(env);
      ...
      case 'o':
        if (useMore == 1) return UsageCheckCtlgSpec(env);
            dbgFileName = util_strsav(util_optarg);
            StreamMgr_print_output(streams,   "Output to file: %s\n", dbgFileName);
            break;
      case 'm':
        if (dbgFileName != NIL(char)) return UsageCheckCtlgSpec(env);
            useMore = 1;
            break;
      default:  return UsageCheckCtlgSpec(env);
    }
  }
  if (argc != util_optind) return UsageCheckCtlgSpec(env);
  ...
  if (formula != NIL(char)) {
    ...
    if (Prop_check_type(PropDb_get_prop_at_index(prop_db,
      ...
      CATCH(errmgr) {
          PropDb_get_prop_at_index(prop_db,
                                   prop_no);
      }
      FAIL(errmgr) {
        status = 1;
    ...
    CATCH(errmgr) {
      PropDb_verify_all_type(prop_db,Prop_Ctlg);
    }
    FAIL(errmgr) {
      status = 1;
    }
...
{
  StreamMgr_ptr streams = STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(streams,  "usage: check_gctlspec [-h] [-m | -o file] [-n number | -p \"gradedctl-expr\"]\n");
  StreamMgr_print_error(streams,  "   -h \t\t\tPrints the command usage.\n");
  StreamMgr_print_error(streams,  "   -m \t\t\tPipes output through the program specified\n");
  StreamMgr_print_error(streams,  "      \t\t\tby the \"PAGER\" environment variable if defined,\n");
  StreamMgr_print_error(streams,  "      \t\t\telse through the UNIX command \"more\".\n");
  StreamMgr_print_error(streams,  "   -o file\t\tWrites the generated output to \"file\".\n");
  StreamMgr_print_error(streams,  "   -n number\t\tChecks only the GRADEDSPEC with the given index number.\n");
  StreamMgr_print_error(streams,  "    -p \"gradedctl-expr\"\tChecks only the given GRADEDCTL formula.\n");
  return 1;
